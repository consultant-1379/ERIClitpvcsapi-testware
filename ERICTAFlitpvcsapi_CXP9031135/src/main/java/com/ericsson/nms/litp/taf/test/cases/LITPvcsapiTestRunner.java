...
public class LITPvcsapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPvcsapiTestRunner.class);
    ...
    @TestId(id = "CXP9031135-1", title = "Upgrade ERIClitpvcsapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpvcsapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("vcsapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031135-2", title = "Run python test cases for ERIClitpvcsapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpvcsapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpvcsapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031135-3", title = "Parse xml outputs from python tests")
